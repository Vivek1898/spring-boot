...
public class Genrator850 {
    ...
    public static void main(String[] args) {
        try {
            ...
            // Obtain Stream to write the EDI document
            OutputStream stream = new FileOutputStream("invoice850.edi");
          ...
          .writeElement("U")
          .writeElement("00401")
          .writeElement(String.format("%09d", 2)) // Pad with leading zeros if needed
          .writeElement("0")
          .writeElement("P")
          .writeElement(":")
          .writeEndSegment();
            ...
            // Write BEG segment
            JSONObject invoiceHeader = jsonData.getJSONObject("Header").getJSONObject("InvoiceHeader");
            writer.writeStartSegment("BEG")
                    .writeElement("00")
                    .writeElement(invoiceHeader.getString("TransactionTypeCode"))
                    .writeElement(invoiceHeader.getString("PurchaseOrderNumber"))
                    .writeElement("") // Shipment ID
                    .writeElement(invoiceHeader.getString("PurchaseOrderDate").replaceAll("-", ""))
                    .writeEndSegment();

            // Write REF segment
            JSONArray references = jsonData.getJSONObject("Header").getJSONArray("References");
            ...
            // Write N1 segment (Buyer)
            JSONArray addresses = jsonData.getJSONObject("Header").getJSONArray("Address");
            JSONObject buyer = addresses.getJSONObject(0);
            writer.writeStartSegment("N1")
                    .writeElement(buyer.getString("AddressTypeCode"))
                    .writeElement(buyer.getString("Name"))
                    .writeElement(buyer.getString("IDCodeQualifier"))
                    .writeElement(buyer.getString("IDCode"))
                    .writeEndSegment();

            // Write N3 segment (Buyer address)
            writer.writeStartSegment("N3")
                    .writeElement(buyer.getString("Address1"))
                    .writeEndSegment();

            // Write N4 segment (Buyer address)
            writer.writeStartSegment("N4")
                    .writeElement(buyer.getString("City"))
                    .writeElement(buyer.getString("State"))
                    .writeElement(buyer.getString("PostalCode"))
                    .writeElement(buyer.getString("Country"))
                    .writeEndSegment();

            // Write N1 segment (Seller)
            JSONObject seller = addresses.getJSONObject(1);
            writer.writeStartSegment("N1")
                    .writeElement(seller.getString("AddressTypeCode"))
                    .writeElement(seller.getString("Name"))
                    .writeElement(seller.getString("IDCodeQualifier"))
                    .writeElement(seller.getString("IDCode"))
                    .writeEndSegment();

            // Write N3 segment (Seller address)
            writer.writeStartSegment("N3")
                    .writeElement(seller.getString("Address1"))
                    .writeEndSegment();

            // Write N4 segment (Seller address)
            writer.writeStartSegment("N4")
                    .writeElement(seller.getString("City"))
                    .writeElement(seller.getString("State"))
                    .writeElement(seller.getString("PostalCode"))
                    .writeElement(seller.getString("Country"))
                    .writeEndSegment();

            // Write PO1 segments (Line Items)
            JSONArray lineItems = jsonData.getJSONObject("Structure").getJSONArray("LineItem");
            for (int i = 0; i < lineItems.length(); i++) {
                JSONObject lineItem = lineItems.getJSONObject(i);
                JSONObject invoiceLine = lineItem.getJSONObject("InvoiceLine");
                writer.writeStartSegment("PO1")
                        .writeElement(invoiceLine.getString("LineNumber"))
                        ...
                        .writeElement("") // Alternate price
                        .writeElement("VC")
                        .writeElement(invoiceLine.getString("VendorPartNumber"))
                        .writeEndSegment();
            }

            // Write CTT segment (Transaction Totals)
            JSONObject summary = jsonData.getJSONObject("Summary");
            
            System.out.println("summary: " + summary);
            writer.writeStartSegment("CTT")
                    .writeElement(String.valueOf(summary.getInt("TotalLineItems")))
                    .writeEndSegment();

            // Write SE segment
            writer.writeStartSegment("SE")
                    .writeElement(String.valueOf(lineItems.length() + 11))
                    .writeElement("0001")
                    .writeEndSegment();
            ...
            writer.writeStartSegment("IEA")
                    .writeElement("1")
                    .writeElement("000000002") // Control Number
                    .writeEndSegment();
